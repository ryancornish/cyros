/**
 * @file port_core.h
 * @brief Cyros CORE port contract (C ABI): everything an architecture provides.
 *
 * This is the part of the port that depends ONLY on the processor core, and is
 * therefore identical across every MCU built on that core. Context switching,
 * interrupt masking, preemption control, the stack guard, TLS and the debug
 * entry points.
 *
 * It deliberately excludes anything an MCU may answer differently, which is
 * `port_mcu.h`: multicore identity and bring-up, the inter-core interrupt, and
 * the whole time source.
 *
 * The split is what makes a layered port LINKABLE. A core layer that defined
 * the MCU functions too would collide with the MCU layer that replaces them,
 * leaving only weak symbols, which silently hijack rather than fail.
 *
 ...
 * for the time driver is separate-but-parallel to this contract (see port_mcu.h).
 */

#ifndef CYROS_PORT_CORE_H
#define CYROS_PORT_CORE_H
