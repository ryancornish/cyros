 ...
 *
 * NOT INLINE, deliberately, and it costs one call/return on a path that already
 * traps to the host for microseconds. A `BKPT` inlined into the caller's line
 * makes that line impossible to STEP OVER in a debugger: gdb single-steps
 * through the line, OpenOCD services the semihosting trap and resumes the core
 * itself, and the step never completes, so the session appears to hang on the
 * source line while the target runs on. As an out-of-line CALL, gdb steps over
 * it by breakpointing the return address and resuming, which is exactly the
 * pattern the trap handling expects.
 */
